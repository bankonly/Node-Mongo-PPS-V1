...
const PublicController = {
    list_course: Catcher(async(req, res) => {
        ...
        const populate = { path: "instructor_id", select: "name desc img cover_img" }

        const cond = { is_publish: true }
        const found_course = await Mongo.find(CourseModel, { condition: cond, paginate: { paginate: req.query }, populate, select, search: { key: search_field, key_word: req.query.q } })
    ...
    list_course_by_type_or_tool: Catcher(async(req, res) => {
        ...
        const found_course = await Mongo.find(CourseModel, { condition: cond, paginate: { paginate: req.query }, select, populate, search: { key: search_field, key_word: req.query.q } })
        return resp.success({ data: found_course });
    }),
