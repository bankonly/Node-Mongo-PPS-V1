...
const InstructorController = {
  ...
  edit_profile: Catcher(async (req, res) => {
    ...
    if (_.isFile(req.files, "img")) {
      const upload_img = await AwsFunc.upload_img({ file: req.files.img, fileType: "jpg", path: Constant.image_path.instrcutor_profile, resize: [800, 256] })
      found_instructor.img = upload_img
    }

    if (_.isFile(req.files, "cover_img")) {
      const upload_img = await AwsFunc.upload_img({ file: req.files.cover_img, fileType: "jpg", path: Constant.image_path.instrcutor_cover_img })
