...
const VideoController = {
    list: Catcher(async(req, res) => {
        ...
        const select = req.query.select || "desc last_watch_time video_time title episode video_path full_video_path snipped_file created_at"
        const cond = { $or: [{ course_id: req.body.target_id }, { course_section_id: req.body.target_id }] }
        const found_video = await Mongo.find(VideoModel, { condition: cond, paginate: { paginate: req.query }, select })
    ...
    create: Catcher(async(req, res, next, opts) => {
        const resp = new Res(res);

        const files = req.files

        const found_course = await Mongo.findById(CourseModel, { _id: req.body.course_id })
        await Mongo.findById(SectionModel, { _id: req.body.course_section_id })
        ...
        found_course.course_duration = found_course.course_duration + parseInt(upload_video.video_info.duration_sec)
        await found_course.save(opts)

        let new_course_video = null
        try {
            new_course_video = await new VideoModel(req.body).save(opts)
        } catch (error) {
    ...
    update: Catcher(async(req, res) => {
        ...
        if (req.body.course_id) await Mongo.findById(CourseModel, { _id: req.body.course_id })
        if (req.body.section_id) await Mongo.findById(SectionModel, { _id: req.body.section_id })
